...
# Generacion de una solución inicial de ejemplo (lista de números)
def generar_solucion_inicial(num_elementos):
    return [random.randint(0, 100) for _ in range(num_elementos)]

# Vecindad de una solucion: perturbación simple (puede ser personalizada)
...
#El codigo para un algoritmo de busqueda tabu puede ser bastante extenso y complejo, ya que implica la implementación de una heurística específica, la definición de estructuras de datos para gestionar la lista tabu y la logica para explorar el espacio de busqueda. 
