def busqueda_voraz_primero_el_mejor(estado_inicial, es_objetivo, generar_sucesores, funcion_evaluacion):
    ...
    return None  # No se encontro una solución
...
# En este ejemplo, el algoritmo de busqueda voraz "Primero el Mejor" se utiliza para encontrar una solución al problema de llegar al estado objetivo 8 desde el estado inicial 0. La funcion de evaluacion funcion_evaluacion mide la distancia entre el estado actual y el estado objetivo. El algoritmo selecciona el sucesor con la menor distancia en cada paso.

#Ten en cuenta que este algoritmo de búsqueda voraz no garantiza la optima solución en todos los casos y puede quedar atrapado en mínimos locales. 
