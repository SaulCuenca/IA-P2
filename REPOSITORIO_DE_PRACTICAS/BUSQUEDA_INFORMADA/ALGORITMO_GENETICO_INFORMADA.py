...
# Funcion para crear una población inicial de individuos aleatorios.
...
# Funcion para seleccionar a los individuos más aptos.
...
# Funcion para realizar el cruce (crossover) entre dos padres y crear un hijo.
def crossover(parent1, parent2):
    crossover_point = random.randint(1, len(parent1) - 1)
    child = parent1[:crossover_point] + parent2[crossover_point:]
    return child

# Funcion para aplicar una mutación a un individuo con una probabilidad dada.
...
for generation in range(num_generations):
    ...
    print(f"Generacion {generation}: {best_individual} (Fitness: {fitness(best_individual)})")
